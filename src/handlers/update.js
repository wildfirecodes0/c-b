...
async function handleChannelMember(update) {
  try {
    ...
    if (status === 'kicked' || status === 'left') {
      // Removals performed by this bot (expiry cron, creator "Remove", cancel) update the DB
      // themselves — reacting here would flip the sub to 'left' and send a bogus
      // "Member Left Channel" alert to the admin.
      const botId = String(process.env.BOT_TOKEN || '').split(':')[0];
      if (botId && String(update.from?.id) === botId) return;
