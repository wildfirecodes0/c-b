...
const { startCronJobs, getCronStatus } = require('./handlers/cron');
...
app.get('/health', (req, res) => {
  ...
  const lastRun = cronStatus.expiry?.lastRunAt;
  res.json({
    status: 'ok', version: '1.0.0', timestamp: Date.now(),
    // If lastRunAt is missing/old the expiry job is not running (e.g. Render free plan asleep)
    expiryJob: { lastRunAt: lastRun || null, lastOkAt: cronStatus.expiry?.lastOkAt || null, lastError: cronStatus.expiry?.lastError || null, lastStats: cronStatus.expiry?.lastStats || null },
  });
});
...
// Make sure the DB has every column the expiry logic needs, THEN start the cron jobs.
ensureSchema()
  .catch((err) => console.error('ensureSchema error:', err.message))
  .finally(() => startCronJobs());
